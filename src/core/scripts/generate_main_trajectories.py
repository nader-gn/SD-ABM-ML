...
from figure_kpi_source_utils import DECISION12_KPIS, UNIT_MAP_24, choose_source_rows, write_csv
...
def load_decision12_timeseries(root: Path):
    ...
    write_csv(audit, root / 'verification' / 'Figure_05_source_audit.csv')
    return df
...
def generate_figure(df: pd.DataFrame, root: Path):
    ...
    out_svg = root / 'figures' / 'Figure 5.svg'
